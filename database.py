...
def init_db():
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS compositions (
        id SERIAL PRIMARY KEY,
        florist_id INTEGER NOT NULL REFERENCES users(id),
        photo_file_id TEXT,
        cost INTEGER NOT NULL DEFAULT 0,
    ...
    migrations = [
        ...
        "ALTER TABLE shifts ADD COLUMN IF NOT EXISTS worked_minutes INTEGER",
        "ALTER TABLE shifts ADD COLUMN IF NOT EXISTS overtime_minutes INTEGER DEFAULT 0",
        "ALTER TABLE tasks ADD COLUMN IF NOT EXISTS title TEXT",
        "ALTER TABLE tasks ADD COLUMN IF NOT EXISTS created_by INTEGER",
        "ALTER TABLE tasks ADD COLUMN IF NOT EXISTS difficulty TEXT DEFAULT 'normal'",
        "ALTER TABLE tasks ADD COLUMN IF NOT EXISTS sent_at TEXT",
        "ALTER TABLE tasks ADD COLUMN IF NOT EXISTS mandatory INTEGER DEFAULT 0",
        "ALTER TABLE tasks ADD COLUMN IF NOT EXISTS snoozed_until TEXT",
        "ALTER TABLE tasks ADD COLUMN IF NOT EXISTS reason_3h_sent INTEGER DEFAULT 0",
...
def create_composition(florist_id, photo_file_id, cost, price):
    conn = get_conn(); cur = conn.cursor()
    cur.execute(
        "INSERT INTO compositions (florist_id,photo_file_id,cost,price) VALUES (%s,%s,%s,%s) RETURNING id",
        (florist_id, photo_file_id, cost, price))
    cid = _one(cur)["id"]
    conn.commit(); cur.close(); conn.close()
    return cid
...
def get_composition(composition_id):
    conn = get_conn(); cur = conn.cursor()
    cur.execute(
        "SELECT c.*, u.name as florist_name FROM compositions c "
        "JOIN users u ON c.florist_id=u.id WHERE c.id=%s", (composition_id,))
    row = _one(cur); cur.close(); conn.close()
    return row

def update_composition(composition_id, **kwargs):
    conn = get_conn(); cur = conn.cursor()
    sets = ", ".join(f"{k}=%s" for k in kwargs)
    vals = list(kwargs.values()) + [composition_id]
...
def count_active_compositions():
    ...
    conn = get_conn(); cur = conn.cursor()
    cur.execute("SELECT COUNT(*) FROM compositions WHERE status='in_vitrina'")
    row = cur.fetchone()
    cur.close(); conn.close()
    return row[0] if row else 0

def get_active_compositions(florist_id=None):
    conn = get_conn(); cur = conn.cursor()
    q = ("SELECT c.*, u.name as florist_name FROM compositions c "
         "JOIN users u ON c.florist_id=u.id WHERE c.status='in_vitrina'")
    params = []
    if florist_id:
        q += " AND c.florist_id=%s"; params.append(florist_id)
    q += " ORDER BY c.created_at"
    cur.execute(q, params)
    rows = _all(cur); cur.close(); conn.close()
    return rows

def get_compositions_for_check(days, field):
    conn = get_conn(); cur = conn.cursor()
    cur.execute(
        f"SELECT c.*, u.name as florist_name, u.telegram_id as florist_tg "
        f"FROM compositions c JOIN users u ON c.florist_id=u.id "
        f"WHERE c.status='in_vitrina' "
        f"AND c.created_at::date = CURRENT_DATE - {days} "
        f"AND c.{field}=0")
    rows = _all(cur); cur.close(); conn.close()
    return rows

def get_month_compositions(year_month, florist_id=None):
    conn = get_conn(); cur = conn.cursor()
    q = ("SELECT c.*, u.name as florist_name FROM compositions c "
         "JOIN users u ON c.florist_id=u.id WHERE c.created_at LIKE %s")
    params = [f"{year_month}%"]
    if florist_id:
        q += " AND c.florist_id=%s"; params.append(florist_id)
    cur.execute(q, params)
...
def create_custom_task(assigned_to, created_by, date, scheduled_time, title, difficulty="normal", mandatory=False, require_photo=False):
    conn = get_conn(); cur = conn.cursor()
    cur.execute(
        "INSERT INTO tasks (type,assigned_to,date,scheduled_time,title,created_by,difficulty,mandatory,require_photo,status) "
        "VALUES ('custom',%s,%s,%s,%s,%s,%s,%s,%s,'pending') RETURNING id",
        (assigned_to, date, scheduled_time, title, created_by, difficulty,
         1 if mandatory else 0, 1 if require_photo else 0))
    tid = _one(cur)["id"]
    conn.commit(); cur.close(); conn.close()
    return tid

def get_due_custom_tasks(date_str, time_str):
    """Разовые задачи, время которых уже наступило и которые ещё не отправлены."""
    conn = get_conn(); cur = conn.cursor()
    cur.execute(
...
def get_missed_mandatory_tasks(date_str):
    """Обязательные задачи за указанную дату, которые так и остались не закрыты (pending/in_hour)."""
    conn = get_conn(); cur = conn.cursor()
    cur.execute(
        "SELECT * FROM tasks WHERE type='custom' AND mandatory=1 AND date=%s "
        "AND status IN ('pending','in_hour')", (date_str,))
    rows = _all(cur); cur.close(); conn.close()
    return rows

def get_month_custom_tasks(year_month, florist_id=None):
    conn = get_conn(); cur = conn.cursor()
    q = "SELECT * FROM tasks WHERE type='custom' AND date LIKE %s"
    params = [f"{year_month}%"]
    if florist_id:
...
def get_open_tasks(florist_id=None):
    ...
    conn = get_conn(); cur = conn.cursor()
    q = ("SELECT t.*, u.name as florist_name FROM tasks t JOIN users u ON t.assigned_to=u.id "
         "WHERE t.type='custom' AND t.status IN ('pending','in_hour')")
    params = []
    if florist_id:
        q += " AND t.assigned_to=%s"; params.append(florist_id)
    q += " ORDER BY t.date, t.scheduled_time"
    cur.execute(q, params)
    rows = _all(cur); cur.close(); conn.close()
    return rows
