...
def create_custom_task(assigned_to, created_by, date, scheduled_time, title, difficulty="normal", mandatory=False):
    conn = get_conn(); cur = conn.cursor()
    cur.execute(
        "INSERT INTO tasks (type,assigned_to,date,scheduled_time,title,created_by,difficulty,mandatory,status) "
        "VALUES ('custom',%s,%s,%s,%s,%s,%s,%s,'pending') RETURNING id",
        (assigned_to, date, scheduled_time, title, created_by, difficulty, 1 if mandatory else 0))
    tid = _one(cur)["id"]
    conn.commit(); cur.close(); conn.close()
    return tid
