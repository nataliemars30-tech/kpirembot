...
def _time_diff_minutes(start_str, end_str):
    """Разница в минутах между двумя строками ЧЧ:ММ, с учётом перехода через полночь."""
    try:
...
async def safe_edit(q, text):
    """Редактирует текст или подпись сообщения — работает и для фото, и для обычных сообщений.
    Раньше код дёргал q.message.edit_text() напрямую, и если кнопка была прикреплена
    к фото (например, из /vitrina, /kompozicii или напоминания дня 4), Telegram кидал
    ошибку и весь остаток функции (в т.ч. уведомление директору) не выполнялся."""
    try:
        if q.message.photo:
...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Настройки
        ...
        task_id  = int(parts[2])
        director = db.get_director()
        fl       = db.get_user_by_id(user["id"]) if user else None
        ...
        if action == "done":
            ...
            # Сохраняем для повторного напоминания через 15 мин
        ...
        if action == "done":
            ...
            await q.message.edit_text(
                "Хорошо! Напомню через час.", reply_markup=None)
            if director:
                await ctx.bot.send_message(director["telegram_id"],
                    f"⏰ {fl['name']}: соберёт букеты через час (на витрине {active_count})")
            # Запоминаем для повторного напоминания
        ...
        # Директор нажала «Ознакомлена» по опозданию
        parts = data.split(":")
        ...
        try:
            ...
            pass
        # Флористу — уведомление что директор видела
        try:
...
async def resend_task_kb(bot, florist, task_type, task_id):
    """Повторно отправить кнопки задачи без создания новой записи в БД."""
    import keyboards as kb
    texts = {
...
async def photo_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx.user_data.get("closing_step"):
        ...
        if step == "receipt":
            ...
            overtime_txt = ""
            start_time = shift.get("receipt_time")
            if start_time:
    ...
    # Обработка причины нехватки букетов
    if "shortage_reason_florist" in ctx.user_data and update.message.text:
        pass  # обрабатывается в text_handler
    ...
    if waiting_receipt or shift_needs_receipt:
        ...
        if late_type is None:
            ...
            # Считаем минуты опоздания
            try:
                t_receipt = datetime.strptime(now_time, "%H:%M")
                t_start   = datetime.strptime("10:03", "%H:%M")
                late_min  = int((t_receipt - t_start).total_seconds() / 60)
            except Exception:
                late_min = 0

            # Флористу — сразу сообщение об опоздании
            await update.message.reply_text(
    ...
    if "pending_task_id" in ctx.user_data:
        ...
        # Проверяем кол-во букетов на витрине после отчёта
...
async def text_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx.user_data.get("new_task_step") == "text":
        ctx.user_data["new_task_title"] = text
    ...
    # Разовая задача — дата вручную (если не нажала кнопку Сегодня/Завтра)
    ...
    # Разовая задача — время напоминания
    if ctx.user_data.get("new_task_step") == "time":
        ...
        title       = ctx.user_data.pop("new_task_title")
        ctx.user_data.pop("new_task_step", None)

        task_id = db.create_custom_task(assigned_to, created_by, task_date, scheduled_time, title, difficulty, mandatory)
        fl = db.get_user_by_id(assigned_to)
        mand_txt = "\n🔴 Обязательно день в день — просрочка = минус KPI" if mandatory else ""
        await update.message.reply_text(
            f"✅ Задача создана!\n«{title}»\n"
            f"{TASK_DIFFICULTY_LABELS.get(difficulty, difficulty)} · {task_date} в {scheduled_time}{mand_txt}\n"
    ...
    # Новая цена продажи композиции (скидка или Flowwow)
...
async def cmd_new_task(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Разовая задача: /zadacha — директор выбирает флориста, флорист ставит себе сам."""
    user = db.get_user(update.effective_user.id)
    if not user:
        return
    if user["role"] == "director":
...
async def cmd_chasy(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Часы и переработки за месяц — для расчёта доплаты."""
    if not is_director(update.effective_user.id): return
...
async def cmd_test_smena(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not is_director(update.effective_user.id): return
    from handlers import shift_prompt
    florists = db.get_florists()
...
async def cmd_test_vitrina(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not workers:
        await update.message.reply_text("Нет флористов на смене сегодня. Сначала открой смену.")
        return
    for f in workers:
        # Для теста — удаляем старую задачу и создаём новую
        conn = db.get_conn(); cur = conn.cursor()
...
async def cmd_test_buket4(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for b in bouquets:
        ...
        if b.get("photo_file_id"):
            await ctx.bot.send_photo(b["florist_tg"] if b.get("florist_tg") else update.effective_user.id,
...
async def cmd_test_buket6(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for b in bouquets:
        ...
        if fl:
            if b.get("photo_file_id"):
                await ctx.bot.send_photo(fl["telegram_id"],
                    photo=b["photo_file_id"], caption=caption,
                    reply_markup=kb.bouquet_check_kb(b["id"]))
            else:
                await ctx.bot.send_message(fl["telegram_id"], caption,
...
async def cmd_test_reminder(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Тест напоминания о смене — имитирует что флорист не открыла смену."""
    if not is_director(update.effective_user.id): return
    florists = db.get_florists()
    for f in florists:
        try:
            await ctx.bot.send_message(f["telegram_id"],
                "🔴 ТЕСТ: Открой смену! Нажми /otkryt")
        except Exception as e:
            log.error(e)
...
async def cmd_debug(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [
        ...
        f"Сегодня: {today}",
        f"Минут от 00:00: {t}",
        f"Окно 10:05-11:00: {10*60+5} — {11*60}",
        f"Попадает в окно: {10*60+5 <= t <= 11*60}",
        "",
    ]
    ...
    for f in florists:
        ...
        lines.append(f"\n{f['name']}:")
        lines.append(f"  Смена сегодня: {'ДА' if has_s else 'НЕТ'}")
        lines.append(f"  Чек сегодня: {'ДА' if has_r else 'НЕТ'}")

    await update.message.reply_text("\n".join(lines))

    # Попробуем отправить тестовое сообщение флористам прямо сейчас
    sent = 0
    for f in florists:
        has_r = db.has_receipt(f["id"], today)
        has_s = db.has_shift(f["id"], today)
        if not has_r:
            try:
                msg = "Открой смену!" if not has_s else "Пришли фото чека!"
                await ctx.bot.send_message(f["telegram_id"], f"ТЕСТ АЛЕРТА: {msg}")
                sent += 1
            except Exception as e:
                await update.message.reply_text(f"Ошибка отправки {f['name']}: {e}")

    await update.message.reply_text(f"Отправила тестовых алертов: {sent}")

    # Показать задачи на сегодня
    lines = [f"\nЗадачи на {today}:"]
    conn = db.get_conn(); cur = conn.cursor()
    cur.execute(
        "SELECT t.type, t.status, t.scheduled_time, u.name "
        "FROM tasks t JOIN users u ON t.assigned_to=u.id "
        "WHERE t.date=%s", (today,))
    tasks = [dict(zip([d[0] for d in cur.description], r)) for r in cur.fetchall()]
    cur.close(); conn.close()
    if tasks:
        for t in tasks:
            lines.append(f"  {t['name']}: {t['type']} {t['scheduled_time']} → {t['status']}")
    else:
        lines.append("  Задач нет")
    now_msk_t = now.hour * 60 + now.minute
    lines.append(f"\nТекущее время (мин): {now_msk_t}")
    lines.append(f"Витрина букетов старт (мин): {14*60} (+5 = {14*60+5})")
    lines.append(f"Условие t >= start+5: {now_msk_t} >= {14*60+5} = {now_msk_t >= 14*60+5}")
    await update.message.reply_text("\n".join(lines))


async def cmd_test_alert(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Тест алерта смены — отправляет напоминание прямо сейчас."""
    if not is_director(update.effective_user.id):
        return
    ...
    for f in florists:
        ...
        if has_r:
            msg = f"{f['name']} — чек уже получен, алерт не нужен"
...
async def cmd_pravila(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    l = lates.get("light",   0)
    m = lates.get("medium",  0)
    h = lates.get("heavy",   0)
    n = lates.get("no_show", 0)
    ...
    lines = [
        ...
        "  До 10:03 = вовремя",
        "  После 10:03 = опоздание",
        "  Закрытие: в 21:00 кнопка «Закрыть смену»",
        "  → фото чека + 2 фото холодильника",
        "",
        "⏰ ОПОЗДАНИЯ И ШТРАФЫ",
        f"  ✅ До 10:03 — вовремя",
        f"  🟡 10:03–10:15 — лёгкое · штраф на {max_light}-й раз (-2 000 ₽)",
        ...
        f"  {bar(l, max_light-1)} Лёгких: {l} / {max_light-1} допустимых (штраф на {max_light}-й)",
        f"  {bar(m, max_medium-1)} Средних: {m} / {max_medium-1} допустимых (штраф на {max_medium}-й)",
        f"  {bar(h, max_heavy-1)} Серьёзных: {h} / {max_heavy-1} допустимых (штраф на {max_heavy}-й)",
        ...
        "  Директор оценивает: Отлично / Норм / Плохо",
        "  День 4 — проверить букет",
        "  День 6 — разобрать букет",
        "",
        sep,
        "📊 KPI (считается каждый месяц)",
        "  ✅ Пройден: ≤3 пропусков по каждому отчёту",
        "  ✅ Качество: менее 30% оценок «Плохо»",
        "  ✅ Опоздания: не превышены пороги",
        "  ⭐ Отлично: 80%+ оценок «Отлично»",
        "  4 месяца подряд → повышение категории",
    ]
...
async def cmd_reset_tasks(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Очистить все задачи за сегодня — для тестирования."""
    if not is_director(update.effective_user.id):
        return
    ...
    conn.commit(); cur.close(); conn.close()
    await update.message.reply_text(f"Все задачи за {today} удалены. Теперь можно тестировать заново.")
