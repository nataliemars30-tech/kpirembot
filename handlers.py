...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Разобрать композицию
        ...
        await q.message.edit_text(f"🗑 Композиция #{composition_id} разобрана — материалы пойдут в новую работу.", reply_markup=None)
        fl = db.get_user_by_id(composition["florist_id"])
        await send_to_director(ctx.bot,
...
async def cmd_pravila(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [
        ...
        sep,
        "🌸 ЕЖЕДНЕВНЫЕ ОТЧЁТЫ",
        "  14:00 — Витрина букетов (фото)",
        "  18:00 — Витрина композиций (фото)",
        "  15:00 — Flowwow через день (фото/скрин)",
