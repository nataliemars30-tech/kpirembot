...
async def cmd_pravila(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not user or user["role"] != "florist":
        return
    await update.message.reply_text("Твои правила и статистика штрафов...")

# --- Заглушки для остальных команд, чтобы бот не падал ---
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE): pass
async def text_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE): pass
...
async def cmd_manual_flowwow(update: Update, ctx: ContextTypes.DEFAULT_TYPE): passimport logging
