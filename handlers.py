...
async def cmd_pravila(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if n >= 1:
        fine = 2000
        warnings.append("После 11:00 — штраф уже начислен!")
    elif h >= max_heavy:
        fine = 2000
        warnings.append(f"Серьёзных опозданий {h} из {max_heavy} — штраф!")
    elif m >= max_medium:
        fine = 2000
        warnings.append(f"Средних опозданий {m} из {max_medium} — штраф!")
    elif l >= max_light:
        fine = 2000
        warnings.append(f"Лёгких опозданий {l} из {max_light} — штраф!")
    ...
    lines = [
        f"Правила опозданий · {month}",
        "",
        "Шкала штрафов:",
        "  До 10:03 — вовремя, штрафа нет",
        f"  10:03–10:15 — лёгкое ({max_light-1} допустимых, штраф на {max_light}-й = -2 000 руб.)",
        f"  10:15–10:30 — среднее ({max_medium-1} допустимых, штраф на {max_medium}-й = -2 000 руб.)",
        f"  10:30–11:00 — серьёзное ({max_heavy-1} допустимое, штраф на {max_heavy}-й = -2 000 руб.)",
        "  После 11:00 — каждый раз -2 000 руб. сразу",
        "",
        f"Твоя статистика за {ym}:",
        f"  {bar(l, max_light-1)} Лёгких (до 10:15): {l} из {max_light-1} (штраф на {max_light}-й)",
        f"  {bar(m, max_medium-1)} Средних (до 10:30): {m} из {max_medium-1} (штраф на {max_medium}-й)",
        f"  {bar(h, max_heavy-1)} Серьёзных (до 11:00): {h} из {max_heavy-1} (штраф на {max_heavy}-й)",
    ...
    if warnings:
        ...
        lines.append(f"Штраф в этом месяце: -{fine} руб.")
    else:
        lines.append("Штрафов нет — так держать!")
