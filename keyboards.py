...
def task_photo_confirm_kb():
    """Нужно ли фото-подтверждение при выполнении задачи."""
    return InlineKeyboardMarkup([[
        InlineKeyboardButton("📷 Нужно фото", callback_data="tphoto:yes"),
        InlineKeyboardButton("✅ Без фото", callback_data="tphoto:no"),
    ]])
...
def bouquet_status_kb(bouquet_id):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔄 Пересобран", callback_data=f"breassemble:{bouquet_id}")],
...
def composition_status_kb(composition_id):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔄 Пересобрана", callback_data=f"creassemble:{composition_id}")],
