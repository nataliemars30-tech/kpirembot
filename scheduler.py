...
async def job_all_task_reminders(app):
    """Единый джоб — каждые 5 минут повторяет задачу с кнопками если нет ответа."""
    now      = now_msk()
    today    = today_msk()
    t        = now.hour * 60 + now.minute
    director = db.get_director()
    ...
    # Динамически читаем время из настроек
    def time_to_min(s):
    ...
    tasks_config = [
        ...
        ("vitrina_compositions",
         time_to_min(db.get_setting("vitrina_compositions_time", "18:00")),
         db.get_setting("vitrina_compositions_time", "18:00")),
        ("flowwow",
         time_to_min(db.get_setting("flowwow_time", "15:00")),
         db.get_setting("flowwow_time", "15:00")),
    ]
    ...
    for task_type, start_min, task_time in tasks_config:
        ...
        for f in workers:
            ...
            if not task:
                continue
            # Повторно отправляем задачу с кнопками
            try:
                await send_task(app.bot, f, task_type, task_time, force=True)
                log.info(f"[reminder] resent task to {f['name']}")
            except Exception as e:
                log.error(e)
            # Директору каждые 15 минут
            if director and elapsed % 15 == 0:
                labels = {
                    "vitrina_bouquets":     f"витрина букетов ({task_time})",
                    "vitrina_compositions": f"витрина композиций ({task_time})",
                    "flowwow":              f"Flowwow ({task_time})",
                }
                try:
                    await app.bot.send_message(director["telegram_id"],
                        f"Нет ответа — {f['name']}, {labels[task_type]}")
                except Exception as e:
                    log.error(e)
...
async def job_all_task_reminders(app):
    """Единый джоб — проверяет все незакрытые задачи каждые 5 минут."""
    now      = now_msk()
    today    = today_msk()
    t        = now.hour * 60 + now.minute
    director = db.get_director()
...
def setup_scheduler(app):
    ...
    sh, sm = parse_time(db.get_setting("shift_start_time", "10:00"))
    vh, vm = parse_time(db.get_setting("vitrina_bouquets_time", "10:30"))
    ch, cm = parse_time(db.get_setting("vitrina_compositions_time", "10:40"))
    fh, fm = parse_time(db.get_setting("flowwow_time", "11:00"))
