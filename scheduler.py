...
async def job_shift_prompt(app):
    """10:00 — кнопка начала смены."""
    await shift_prompt(app.bot, db.get_florists())


async def job_shift_reminder(app):
    """Каждые 5 минут с 10:05 до 11:00 — напоминать если нет чека."""
    now   = datetime.now()
    today = date.today().isoformat()

    # Работаем только в окне 10:00–11:00
    if not (10 * 60 + 5 <= now.hour * 60 + now.minute <= 11 * 60):
        return
    ...
    for f in florists:
        ...
        if has_r:
            continue  # Чек уже есть — не беспокоим

        t = now.hour * 60 + now.minute

        if not has_s:
            # Смена вообще не открыта
            florist_msg  = "🔴 Открой смену! Нажми /otkryt"
            director_msg = f"🔴 {f['name']} не открыла смену"
        else:
            # Смена открыта но нет чека
            florist_msg  = "⏰ Пришли фото чека с терминала!"
            director_msg = f"⚠️ {f['name']} — нет чека с терминала"

        try:
            await app.bot.send_message(f["telegram_id"], florist_msg)
        except Exception as e:
            log.error(e)

        # Директору — раз в 10 минут, не каждые 5
        if director and t % 10 == 5:
            try:
...
async def job_task_reminder(app, task_type, scheduled_hour, scheduled_min):
    """Напоминание по задаче если флорист не ответила."""
    today    = date.today().isoformat()
    now      = datetime.now()
    now_min  = now.hour * 60 + now.minute
    task_min = scheduled_hour * 60 + scheduled_min

    # Напоминаем только в окне: через 20 мин и через 40 мин после задачи
    elapsed = now_min - task_min
    if elapsed not in range(20, 60):
        return
    ...
    labels = {
        "vitrina_bouquets":     "витрину букетов",
        "vitrina_compositions": "витрину композиций",
        "flowwow":              "Flowwow",
    }
    ...
    for f in workers:
        ...
        if not task:
            continue  # Задача уже выполнена или нет задачи

        try:
            await app.bot.send_message(f["telegram_id"],
                f"⏰ Напоминание: не забудь отчитаться по {label}!")
        except Exception as e:
            log.error(e)

        if director and elapsed == 30:
            try:
                await app.bot.send_message(director["telegram_id"],
                    f"⚠️ {f['name']} не ответила на задачу: {label} ({scheduled_hour:02d}:{scheduled_min:02d})")
            except Exception as e:
                log.error(e)
...
async def job_bouquet_check_6day(app):
    ...
    for b in bouquets:
        try:
            ...
            if director:
                await app.bot.send_message(director["telegram_id"],
                    f"🔴 Букет #{b['id']} — {b['florist_name']} — {days} дней!")
        except Exception as e:
            log.error(e)
...
def setup_scheduler(app):
    ...
    # Смена — кнопка и напоминания каждые 5 минут до 11:00
    ...
    scheduler.add_job(wrapn(job_shift_reminder),   "interval", minutes=5,
                      start_date=datetime.now().replace(hour=10, minute=5, second=0))
    ...
    scheduler.add_job(wrap(job_task_reminder, "vitrina_bouquets",     vh, vm), "interval", minutes=10)
    scheduler.add_job(wrap(job_task_reminder, "vitrina_compositions", ch, cm), "interval", minutes=10)
    scheduler.add_job(wrap(job_task_reminder, "flowwow",              fh, fm), "interval", minutes=10)
