...
async def job_shift_reminder(app):
    """
    Каждые 5 минут с 10:05 до 11:00 МСК.
    Напоминает флористу открыть смену или прислать чек.
    Директору — каждые 10 минут.
    """
    now   = now_msk()
    ...
    for f in florists:
        ...
        if not has_s:
            # Смена не открыта — напоминаем весь день пока не откроет
            florist_msg  = "Открой смену! Нажми /otkryt или кнопку «Начать смену»"
            director_msg = f"{f['name']} не открыла смену"
        else:
            # Смена открыта но нет чека — напоминаем весь день
            florist_msg  = "Не забудь прислать фото чека с терминала!"
...
async def job_task_reminder(app, task_type):
    """
    Каждые 10 минут проверяет незакрытые задачи.
    Флористу — напоминание каждые 20 мин.
    Директору — раз в 30 мин.
    """
    now    = now_msk()
    today  = today_msk()
    t      = now.hour * 60 + now.minute

    # Определяем окно для каждого типа задачи
    windows = {
        "vitrina_bouquets":     (14 * 60, 15 * 60),
        "vitrina_compositions": (18 * 60, 19 * 60),
        "flowwow":              (15 * 60, 16 * 60),
    }
    start_min, end_min = windows.get(task_type, (0, 0))
    if not (start_min + 20 <= t <= end_min):
        return
    ...
    for f in workers:
        ...
        # Директору каждые 30 минут
        if director and elapsed % 30 == 0:
            try:
...
def setup_scheduler(app):
    ...
    scheduler.add_job(wrap(job_task_reminder, "vitrina_bouquets"),     "interval", minutes=10)
    scheduler.add_job(wrap(job_task_reminder, "vitrina_compositions"), "interval", minutes=10)
    scheduler.add_job(wrap(job_task_reminder, "flowwow"),              "interval", minutes=10)
